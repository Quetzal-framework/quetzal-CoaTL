  ...
  namespace coalescence
  {
    namespace merger_policy
    {
      ...
      struct BinaryMerger
      {
        ...
        static auto
        merge(BidirectionalIterator first, BidirectionalIterator last, unsigned int N, T init, BinaryOperation const& op, Generator& g)
        {
      ...
      ///
      /// @tparam SpectrumSamplingPolicy The policy used to generate an OccupancySpectrum: on_the_fly or in_memoized_distribution
      ///
      ...
      struct SimultaneousMultipleMerger
      {
        ...
        template<class BidirectionalIterator, class T, class BinaryOperation, class Generator>
        static auto
        merge(BidirectionalIterator first, BidirectionalIterator last, unsigned int N, T const& init, BinaryOperation const& op, Generator& g)
        {
          assert(N >= 1 && "Population size should be positive for evaluating coalescence probability" );
          assert(std::distance(first, last) > 1 && "Coalescence should operate on a range containing more than one element.");
          unsigned int k = std::distance(first, last);
          return algorithm::simultaneous_multiple_merge(first, last, init, SpectrumSamplingPolicy::sample(k, N, g), op, g);
        }
        ///
        /// @brief merges multiple randomly selected elements in a range.
        ///
