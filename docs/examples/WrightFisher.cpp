...
// You can give shortnames to namespace for brievity:
namespace geo = quetzal::geography;
namespace coa = quetzal::coalescence;
namespace engine = quetzal::engine;
int main()
{
  // Initalize a random number generator with a source of entropy
  std::random_device rd;
  std::mt19937 gen(rd());

  // Declaring simulation policies (policy-based design)
  using coalescence_policy = coa::newick::policy::distance_to_parent<>;

  using simulator_type = engine::wright_fisher<coalescence_policy>;
