...
int main(){
	...
	std::unordered_map<int, std::vector<double>> coeffs = { {0,{0.0, 0.1, 0.9, 0.0}}, 
															{1,{0.4, 0.1, 0.2, 0.3}}, 
															{2,{0.0, 0.3, 0.6, 0.1}} };
	FuzzyPartition<int> A(coeffs);
	assert( A.elements() == std::unordered_set<int>({0,1,2}) );
	assert( A.clusters() == std::unordered_set<int>({0,1,2,3}) );
