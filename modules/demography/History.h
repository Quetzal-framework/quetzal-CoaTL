...
namespace demography {
  ...
  struct strategy {
    using individual_based = unsigned int;
    using mass_based = double;
  };
  ...
  class History<Space, Time, quetzal::demography::strategy::individual_based>{

  public:

    using strategy_type = quetzal::demography::strategy::individual_based;
    using flow_type = Flow<Space, Time, unsigned int>;
        ...
        for(auto x : m_sizes->definition_space(t) )
        {
          ...
          if(N_tilde >= 1)
          {
            for(unsigned int ind = 1; ind <= N_tilde; ++ind)
            {
              ...
              m_flows->add_to_flux_from_to(x, y, t, 1);
  ...
  template<typename Space, typename Time>
  class History<Space, Time, quetzal::demography::strategy::mass_based>{

  public:

    using flow_type = Flow<Space, Time, double>;
    using N_type = PopulationSize<Space, Time, double>;
    using coord_type = Space;
    using time_type = Time;
    using discrete_distribution_type = quetzal::random::DiscreteDistribution<coord_type>;
    using backward_kernel_type = quetzal::random::TransitionKernel<time_type, discrete_distribution_type>;
  ...
  public:
    ...
    History(coord_type const& x, time_type const& t, double N):
    m_sizes(std::make_unique<N_type>()),
    m_flows(std::make_unique<flow_type>()),
    m_kernel(std::make_unique<backward_kernel_type>())
    {
    ...
    void expand(unsigned int nb_generations, Growth sim_growth, Dispersal kernel, Generator& gen)
    {
      for(unsigned int g = 0; g < nb_generations; ++g)
      {
        ...
        for(auto x : m_sizes->definition_space(t) )
        {
          ...
          {
            double m = kernel(x, y, t);
            m_flows->set_flux_from_to(x, y, t, m*N_tilde);
            m_sizes->operator()(y, t_next) += m*N_tilde;
          }
        }
      }
    }
