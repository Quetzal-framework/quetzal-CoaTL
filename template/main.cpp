...
int main()
{
    ...
    // lightweight functors for suitability and digital elevation models that return empty optionals where NA are
    // encounters
    ...
    auto suit = quetzal::expressive::use([&](location_type x, time_type t) { return suit_view(x, t).value_or(0.0); });
    auto elev = quetzal::expressive::use([&](location_type x, time_type t) { return elev_view(x, t).value_or(0.0); });
    ...
    std::cout << "Carrying capacity K( x = " << x << ", t = " << t << " ) = " << capacity_with_rafting(x, t)
              << std::endl;
